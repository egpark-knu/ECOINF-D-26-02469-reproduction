"""Substantive verification helpers for P2c v4."""
...
def verify_output(root: Path, rebuild_comparison: dict | None = None) -> dict:
    ...
    required = [
        ...
        "reports/M3_matchups.md", "reports/M4_spatial.md", "reports/P2c_REPORT.md",
    ]
    ...
    _gate(gates, "source_hashes_and_freeze_identity", source_ok, {
        ...
        "packet_sha256": actual_hashes["v4_task_packet"], "drift_roles": sorted(k for k in EXPECTED_HASHES if actual_hashes.get(k) != EXPECTED_HASHES[k]),
    })

    freeze = source_paths()["freeze"][1]
    packet = source_paths()["v4_task_packet"][1]
    ...
    joined_reports = "\n".join((root / f"reports/{name}").read_text(encoding="utf-8") for name in ["M3_matchups.md", "M4_spatial.md", "P2c_REPORT.md"])
