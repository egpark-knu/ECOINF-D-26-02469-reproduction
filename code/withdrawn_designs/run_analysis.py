#!/usr/bin/env python3
"""Run the frozen withdrawn designs M5/M8/M9 branches into a fresh revision-local directory."""
...
DEFAULT_OUTPUT_PARENT = Path(os.environ.get(
    "WITHDRAWN_OUTPUT_PARENT", str(REPOSITORY_ROOT / "reproduction_output/withdrawn_designs")
))
...
def assert_output_jail(output_root: Path, allowed_parent: Path | None = None) -> Path:
    ...
    if root == allowed or allowed not in root.parents:
        raise ValueError(f"output root escapes withdrawn-designs output jail: {root}")
    if root.exists():
