#!/usr/bin/env python3
"""Render the required standalone withdrawn-designs branch and analysis reports from a verified run."""
...
def report_phase(run: Path, base: Path, amendment: Path) -> str:
    ...
    return f"""# withdrawn designs quantitative design and inference report
...
1. Inspected the analysis specification, source inventory, raw daily panel, annual
   proxy panel, chronology/gap/coverage tables, eligibility evidence, historical
   analysis code and output, and the saved primary records. Every used source is hashed in `{run / 'source_manifest.json'}`.
...
- code/tests: `{run.parents[3] / 'code/withdrawn_designs'}`
...
def main(args: argparse.Namespace) -> None:
    ...
    for path, content in outputs.items():
        ...
        print(path)
    code_root = args.campaign / "revision_1/03_analysis/code/withdrawn_designs"
    deliverables = [
        *outputs.keys(),
        args.base.resolve(),
    ...
    delivery_path = args.campaign / "revision_1/03_analysis/output/withdrawn_designs/delivery_manifest.json"
