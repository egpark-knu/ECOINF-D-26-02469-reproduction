...
from run_analysis import _attempt_model_row, _m9_verdict, assert_output_jail
...
class RunnerAxisTests(unittest.TestCase):
    def test_output_jail_normalizes_relative_allowed_path_before_parent_walk(self):
        with tempfile.TemporaryDirectory() as directory:
            allowed = Path(directory) / "withdrawn designs"
    ...
    def test_failed_model_is_recorded_without_fabricated_estimate(self):
        ...
        with patch("run_analysis._model_row", side_effect=ValueError("logit failed to converge")):
