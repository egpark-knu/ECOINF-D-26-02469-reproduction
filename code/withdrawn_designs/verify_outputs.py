#!/usr/bin/env python3
"""Fail-closed, branch-specific verification for a completed withdrawn designs analysis run."""
...
DEFAULT_RUNS_ROOT = Path(os.environ.get(
    "WITHDRAWN_RUNS_ROOT", str(REPOSITORY_ROOT / "reproduction_output/withdrawn_designs/runs")
))
...
def verify(args: argparse.Namespace) -> dict:
    ...
    if allowed not in root.parents or not root.is_dir() or root.is_symlink():
        raise ValueError("run root is not a real directory inside the withdrawn designs run jail")
    ...
    if failed:
        raise ValueError(f"withdrawn designs verification failed: {failed}")
