#!/usr/bin/env python3
"""sampling frame — independent reproduction of the exact V00–V07 assignment sign-flip inference.

Estimator and gates are fixed by 03_analysis/frozen_protocols/sampling_frame_evidence_lock.md.
...
Read-only on every historical input. Writes only under 03_analysis/output/sampling_frame/.
"""
...
BASE = Path(os.environ.get("SAMPLING_FRAME_SOURCE_ROOT", str(REPOSITORY_ROOT / "data/sampling_frame/source_inputs")))
OUT = Path(os.environ.get("SAMPLING_FRAME_OUT", str(REPOSITORY_ROOT / "reproduction_output/sampling_frame")))
...
def main() -> None:
    ...
    for vid in ["V00", "V01", "V02", "V03"] + GEE_VARIANTS:
        ...
        row = {
            ...
            "secondary_inference": "one-sample Student-t interval and one-sample t-test on pair-level DiD (method identified and exactly reproduced by sampling frame; the Round_2 label 'cluster/wild-bootstrap-style' is inaccurate)",
    ...
    ver = {
        "protocol": "sampling_frame_evidence_lock.md",
