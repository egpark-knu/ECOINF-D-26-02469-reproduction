...
WORK = Path(os.environ.get("HYDRO_WORK", str(REPOSITORY_ROOT / "reproduction_output/hydrologic_robustness")))
...
def permute_tau(frame: pd.DataFrame, rng: np.random.Generator, basin_year: bool) -> pd.Series:
    group_col = "basin_year" if basin_year else "year"
    tmp = frame.copy()
    if group_col == "basin_year" and "basin_year" not in tmp.columns:
        tmp["basin_year"] = tmp["river"].astype(str) + "::" + tmp["year"].astype(str)
    return tmp.groupby(group_col)["tau_days"].transform(lambda x: rng.permutation(x.to_numpy()))
...
def single_outcome_ri(panel: pd.DataFrame, season: str, outcome_key: str, standardized: bool, basin_year: bool = False) -> tuple[float, int]:
    ...
    for _ in range(N_PERM):
        perm = sub.copy()
        perm["tau_days"] = permute_tau(perm, rng, basin_year)
...
def stacked_interaction(panel: pd.DataFrame, season: str, basin_year: bool = False) -> Fit:
...
def stacked_interaction_ri(panel: pd.DataFrame, season: str, basin_year: bool = False) -> tuple[float, int]:
    rng = np.random.default_rng(stable_seed("stack_ri", season, basin_year))
    obs = stacked_interaction(panel, season, basin_year).beta
    vals = []
    sub = panel.loc[panel["season_scope"] == season].copy()
    for _ in range(N_PERM):
        perm = sub.copy()
        perm["tau_days"] = permute_tau(perm, rng, basin_year)
        vals.append(stacked_interaction(perm, season, basin_year).beta)
    arr = np.array(vals, dtype=float)
    return float((np.sum(arr >= obs) + 1) / (len(arr) + 1)), int(len(arr))
...
def make_model_outputs(panel: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for season in SEASON_LABELS:
        for basin_year in [False, True]:
            fit = stacked_interaction(panel, season, basin_year=basin_year)
            ri_p, n_perm = stacked_interaction_ri(panel, season, basin_year=basin_year)
...
def write_interaction_report(models: pd.DataFrame, inter: pd.DataFrame) -> None:
    ...
    inter_primary = inter[inter["fixed_effects"] == "weir + year"].copy()
    inter_display = inter_primary.copy()
    ...
    annual = inter_primary.loc[inter_primary["season_scope"] == "annual_all_samples"].iloc[0]
    bloom = inter_primary.loc[inter_primary["season_scope"] == "bloom_season_06_10"].iloc[0]
    ...
    lines.append(
        "Basin-by-year robustness estimates are included in `standardized_tau_models.csv` and `interaction_basinyear.md`; "
        "they add basin-year fixed effects in place of common year fixed effects while retaining weir fixed effects."
    )
    lines.append("")
...
def write_basinyear_report(models: pd.DataFrame, inter: pd.DataFrame) -> None:
    single = models[models["model_family"] == "z_standardized_log1p_outcome_basin_year_robustness"].copy()
    side = inter[inter["fixed_effects"].isin(["weir + year", "weir + basin-by-year"])].copy()
    single["ri_p_right_positive_tau"] = single["ri_p_right_positive_tau"].map(journal_p)
    side["ri_p_right_cyano_gt_chla"] = side["ri_p_right_cyano_gt_chla"].map(journal_p)
    annual_by = inter[
        (inter["season_scope"] == "annual_all_samples") & (inter["fixed_effects"] == "weir + basin-by-year")
    ].iloc[0]
    survives = (
        annual_by["interaction_beta_cyano_minus_chla"] > 0
        and annual_by["ri_p_right_cyano_gt_chla"] < 0.05
        and annual_by["cluster_ci_low"] > 0
        and annual_by["cluster_ci_high"] > 0
    )
    reading = (
        "Headline stands as basin-robust: the annual basin-by-year interaction is positive, the primary randomization "
        "p-value is below 0.05, and the weir-clustered 95% CI excludes zero."
        if survives
        else "Headline must be downgraded: the annual basin-by-year interaction does not satisfy the precommitted survival rule."
    )
    lines = [
        "# Basin-by-Year Stacked Specificity Interaction",
        "",
        "Requested specification: `outcome_std ~ log1p(tau) * outcome_type + weir FE + basin:year FE`.",
        "Primary inference permutes residence time within basin-by-year blocks; intervals are weir-clustered 95% CIs.",
        "",
        "## Side-by-side stacked interaction",
        "",
        side[
            [
                "season_label",
                "fixed_effects",
                "tau_permutation_blocks",
                "interaction_beta_cyano_minus_chla",
                "cluster_ci_low",
                "cluster_ci_high",
                "ri_p_right_cyano_gt_chla",
        ...
        "",
        "## Basin-by-year per-outcome standardized slopes",
        "",
        single[
            [
                "season_label",
        ...
        "",
        "## Precommitted reading",
        "",
        reading,
        "",
    ]
    (OUT / "interaction_basinyear.csv").write_text(side.to_csv(index=False), encoding="utf-8")
    (OUT / "interaction_basinyear.md").write_text("\n".join(lines), encoding="utf-8")
...
def write_figure(models: pd.DataFrame, inter: pd.DataFrame) -> None:
    ...
    for _, row in inter.iterrows():
        note.append(
            f"{row['season_label']} ({row['fixed_effects']}): interaction={row['interaction_beta_cyano_minus_chla']:.2f}, "
...
def main() -> None:
    ...
    write_basinyear_report(models, inter)
    ...
    print(f"Wrote {OUT / 'interaction_basinyear.md'}")
