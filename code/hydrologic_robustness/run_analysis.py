#!/usr/bin/env python3
"""hydrologic robustness — reproduction gate, hydrologic robustness (M02f/g), basin inference (M06).

Specifications are fixed by 03_analysis/frozen_protocols/hydrologic_robustness_freeze.md sections 6-8.
...
def main() -> None:
    ...
    if not reproduction_gate(panel):
        ...
        pd.DataFrame(log).to_json(OUT / "run_log.json", orient="records", indent=2)
    ...
    # BASELINE on the hydrologic robustness frame (z-scaled tau) for like-for-like comparison
    for season in SEASONS:
