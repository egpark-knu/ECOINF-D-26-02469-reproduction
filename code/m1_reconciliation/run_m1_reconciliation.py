#!/usr/bin/env python3
"""Execute the frozen M1 reconciliation M1 plan in a fresh revision-local run directory."""
...
OUTPUT_PARENT = Path(os.environ.get(
    "M1_OUTPUT_PARENT", str(REPOSITORY_ROOT / "reproduction_output/m1_reconciliation/runs")
))
LOG_PARENT = Path(os.environ.get(
    "M1_LOG_PARENT", str(REPOSITORY_ROOT / "reproduction_output/m1_reconciliation/logs")
))
...
HISTORICAL_PATHS = [
    REPOSITORY_ROOT / "code/m1_reconciliation/vendor/hardening_specificity_analysis__c895385a.py",
    REPOSITORY_ROOT / "data/insitu_annual_analysis_panel.csv",
    REPOSITORY_ROOT / "data/m1_reconciliation/runs/20260815T042826Z_c2ac8933/legacy/standardized_tau_models.csv",
    REPOSITORY_ROOT / "data/m1_reconciliation/runs/20260815T042826Z_c2ac8933/legacy/specificity_interaction.csv",
]
    ...
    try:
        with stdout_path.open("w", encoding="utf-8") as stdout_handle, stderr_path.open("w", encoding="utf-8") as stderr_handle:
            with redirect_stdout(stdout_handle), redirect_stderr(stderr_handle):
                print(f"M1 reconciliation run started: {run_root.name}")
