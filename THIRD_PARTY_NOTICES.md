# Third-Party Notices and Acknowledgements

This reproducibility package relies on datasets and software governed by their respective licenses.

1. **Hydrological Data (K-water)**:
   - Source: `water.or.kr`
   - Terms: Subject to Korea Open Government License (KOGL) terms. Not redistributed in raw form.

2. **Water Quality Data (NIER / WEIS)**:
   - Source: `water.nier.go.kr` / `data.go.kr`
   - Terms: Free use with attribution (KOGL).

3. **Copernicus Sentinel-2**:
   - Source: European Space Agency / Copernicus via Google Earth Engine.
   - Terms: Free, full, and open access. "European Union's Copernicus Sentinel data [Year]".

4. **JRC Global Surface Water**:
   - Source: EC JRC via Google Earth Engine.
   - Terms: Free, full, and open access. Cite Pekel et al. (2016).
