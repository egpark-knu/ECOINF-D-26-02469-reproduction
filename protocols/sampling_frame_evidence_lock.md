# sampling frame EVIDENCE LOCK — mask-variant uncertainty transfer and ED-03 source closure
...
June 2026 and already exists on disk. sampling frame reproduces it, reconciles it, and makes it
machine-readable for the manuscript. sampling frame does **not** select a new method, does not fit a new
...
sampling frame writes its **own independent implementation**. The historical script is read to define the
...
**If G3–G6 fail for any variant, sampling frame reports a reproduction failure and does not publish a
...
cluster/wild-bootstrap-style CI/p retained for completeness". sampling frame will attempt to locate the
...
Writes are confined to `03_analysis/frozen_protocols/sampling_frame_evidence_lock.md`,
`03_analysis/code/sampling_frame/**`, `03_analysis/output/sampling_frame/**`,
