# hydrologic robustness FREEZE — hydrologic robustness and basin-preserving inference
...
**Decision: vendor the 27,357 B `2526e1bf…` version** as the hydrologic robustness model module. It
is a strict superset for the estimators hydrologic robustness needs, and it is the version the
...
**τ definition, as built by the submitted pipeline** (not re-derived by hydrologic robustness):
...
If any point estimate fails to reproduce, **the run halts** and hydrologic robustness reports a
...
- `03_analysis/code/hydrologic_robustness/` — vendored module + runnable scripts
- `03_analysis/output/hydrologic_robustness/` — machine-readable estimates, sample accounting, logs
